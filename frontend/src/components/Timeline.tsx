...
export function Timeline() {
  ...
  const dndOverRef = useRef<null | { x: number; y: number; kind: 'media' | 'sticker' }>(null)
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < tracks.length; i++) {
      ...
      ctx.fillStyle = t.muted || locked ? '#5a5a64' : '#9b9ba5'
      ctx.font = '11px var(--font-ui)'
      ctx.fillText(t.label ?? t.id, 22, y + trackHeight / 2 + 4)
      // Locked-state cue: small padlock in the label cell's top-right corner
      // (set_track_locked existed with no visual anywhere) + dimmed label.
      if (locked) {
        ctx.font = '9px var(--font-ui)'
        ctx.fillText('🔒', labelWidth - 14, y + 12)
      }
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < tracks.length; i++) {
      ...
      ctx.font = '11px var(--font-ui)'
      ctx.fillText(t.label ?? t.id, 22, y + trackHeight / 2 + 4)
      // Same locked-state padlock as the main canvas's label draw — this
      // sticky copy is the one actually visible once scrolled.
      if (locked) {
        ctx.font = '9px var(--font-ui)'
        ctx.fillText('🔒', labelWidth - 14, y + 12)
      }
  ...
  useEffect(() => {
    ...
    // twin, so the current time is readable while scrubbing here. Flips to
    // the left side of the line near the content's right edge.
    const tcode = playhead < 60
  ...
  useEffect(() => {
    ...
    if (dnd && (!drag || drag.kind === 'playhead')) {
      ...
      if (ti >= 0 && dnd.x > labelWidth) {
        ...
        const ok = dnd.kind === 'sticker' || laneAcceptsMediaClip(t.type)
        const ty = trackY(ti)
        ...
        const caption = dnd.kind === 'sticker'
          ? 'Drop to add sticker'
          : ok
            ? (t.type === 'video' && t.id !== 'v1' ? `Add to ${name} (PIP overlay)` : `Add to ${name}`)
            : `Media can't go on "${name}" — will land on Main video`
        ctx2.font = '10px var(--font-ui)'
        const capW = ctx2.measureText(caption).width
        const capX = Math.max(labelWidth + 4, Math.min(lx + 8, contentW - capW - 10))
        ctx2.fillStyle = 'rgba(14,14,16,0.85)'
        ctx2.fillRect(capX - 4, ty + 3, capW + 8, 14)
        ctx2.fillStyle = ok ? dv.ACCENT : dv.ACCENT_BAD
        ctx2.fillText(caption, capX, ty + 13)
      }
      ctx2.restore()
      return
    }
  ...
  function onCanvasDragOver(e: React.DragEvent) {
    if (e.dataTransfer.types.includes('application/x-vai-src')
      ...
      dndOverRef.current = {
        ...
        kind: e.dataTransfer.types.includes('application/x-vai-emoji') ? 'sticker' : 'media',
      }
      if (dragRafRef.current == null) {
        dragRafRef.current = requestAnimationFrame(() => {
          dragRafRef.current = null
  ...
  async function onCanvasDrop(e: React.DragEvent) {
    e.preventDefault()
    dndOverRef.current = null
    setDragTick((n) => n + 1)  // repaint overlay now — drop fires no dragleave
  ...
  function onLabelMouseMove(e: React.MouseEvent) {
    ...
    for (let i = 0; i < tracks.length; i++) {
      ...
      if (y >= ty && y <= ty + trackHeight) {
        const t = tracks[i]
        const state = [t.muted ? 'muted' : '', isTrackLocked(t) ? 'locked' : '']
          .filter(Boolean).join(' · ')
        tip = `${t.label ?? t.id} — ${trackPurpose(t)}${state ? `\n(${state})` : ''}\nClick M to mute/unmute`
        overMute = x >= 6 && x <= 18
          && y >= ty + trackHeight / 2 - 6 && y <= ty + trackHeight / 2 + 6
        break
      }
    }
